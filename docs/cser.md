...
<img src="https://cdn.tonycrane.cc/turing2022/images/bTmxcFAXdHNgWS7.jpeg" style="width:45%;"/>
...
对于现在开始学习 CS 的同学，学会使用 AI 和 Agent 已经不是一项可有可无的“加分技能”，而是必须补上的基本能力。就像我们需要学会使用搜索引擎、Git 和命令行一样，越早了解这些工具，就越早能够把时间用在真正需要思考的地方。

没必要一开始对 AI 抱有芥蒂，尝试使用 AI 和 Agent 后你就会发现它实在是太好用了，它可以极大地减少重复性的工作让你把中心放在思考而不是反复敲代码上。

如果你只用过豆包或 DeepSeek，甚至完全没有接触过这些工具，也不用担心。它们并不是什么只有“技术大佬”才能使用的东西，你可以将任何你需要询问的问题与 AI 交流，甚至可以让 AI 来教你如何使用 AI 与 Agent。

### AI 是什么？

你可以先把现在的 AI 理解成一个“读过很多东西，而且很能聊的工具”。你向它输入一段话，它可以根据你的要求解释概念、整理文字、翻译资料、阅读代码，或者和你一起分析问题。

豆包、ChatGPT、Claude、Gemini、DeepSeek 等都属于这类工具，它们背后通常是所谓的“大语言模型”。刚开始并不需要研究模型的具体原理，也不必纠结哪个名字听上去最厉害，先挑一个能正常使用的工具亲自试一试即可。

不过，AI 说话流畅并不代表它真的知道答案。它有时会出现幻觉，编出不存在的函数、论文和网址。因此，使用 AI 时也应该保留一定的质疑。

### Agent 又是什么？

普通的对话式 AI 主要负责“告诉你怎么做”，而 Agent 可以在得到允许后直接“动手做”。例如，你在使用 AI 来修改代码的时候还需要把代码一次次复制粘贴给 AI 的对话框；但是使用 Agent 时，它可以直接在你的电脑上阅读你写代码的文件，并且直接进行更改，可以避免重复地复制粘贴的行为。

这听起来很方便，但也意味着 Agent 犯错时，影响的不再只是一段聊天记录。它可能改坏文件、装错依赖，甚至误删内容。因此，第一次使用 Agent 时，不要一上来就把整个项目扔给它自由发挥，更不要因为它看起来很自信就一路点击“允许”。

### Agent 和大模型是什么关系？

可以把大模型理解成 Agent 的“大脑”。GPT、Claude、Gemini、DeepSeek 等大模型负责读懂你的要求、阅读代码并判断下一步应该做什么，但只靠一个大模型，通常只能在对话框里给你回复。

Agent 则是在大模型外面接上了“眼睛和手”：读取文件相当于看东西，修改代码相当于动手，终端和浏览器则是它可以使用的工具。Agent 还会把这些步骤连起来——先查看项目，再修改代码，然后运行程序；如果出现新的报错，就继续分析和修改。最简单的理解就是：
> **Agent = 大模型 + 可以使用的工具 + 连续完成任务的能力**

同一个 Agent 有时可以选择不同的大模型，而同一个大模型也可以被放进不同的 Agent 中。模型决定它“想得怎么样”，Agent 本身则决定它能看到什么、能操作什么。刚开始使用时不需要研究太多原理，记住 Agent 是“能够使用工具的大模型”就足够了。

目前比较常见的 Agent 有: Codex 、Claude Code 、GitHub Copilot Agent 等。

### AI 可以帮我们做什么？

最简单的用法就是：**哪里卡住了，就把哪里拿去问 AI。**

比如学习数理时，你可以把题目和自己的草稿拍下来，让 AI 读你的字迹，看看是从哪一步开始出错的。这里当然不是鼓励大家直接抄答案。
写代码时就更方便了。代码无法运行、终端突然冒出一片红字、软件装不上、环境配不好，甚至电脑弹出一个完全看不懂的提示，都可以截图或者复制给 AI。它不一定一次就能解决，但至少能先告诉你这些报错大概是什么意思，以及接下来可以试什么。

平时还可以让 AI：

* 把课上没听懂的概念换一种更简单的说法；
* 帮你总结一篇很长的资料，或者翻译英文文档；
* 根据考试范围帮你整理复习提纲、出几道练习题；
* 帮你想报告、展示或小项目可以从哪里开始；
* 检查文字是否通顺，或者把一段乱七八糟的笔记整理清楚。

AI 最好用的地方，不是直接把答案塞给你，而是让你在“不知道下一步怎么办”的时候，至少有一个可以继续往下走的方向。

### 怎样向 AI 提问？

好的 prompt（提示词）可以提升大模型回答的质量，所以正确地向 AI 提问是使用 AI 时最能够直接提升效率与体验的部分。
使用时不需要专门去背各种“高级提示词”，就把 AI 当成一个人，正常说话即可。不过，问题说得越清楚，它越容易帮上忙。

问问题时尽量做到下面三点：

1. **先说自己的情况**：比如“我刚学 C 语言，还没学过指针”；
2. **把信息发完整**：代码、题目、报错和自己的做法都尽量发出来，不要只说一句“怎么不行”；
3. **告诉它你想要什么**：是想直接解决问题，还是只要一个提示，或者希望它从最基础的地方开始解释。

例如，可以直接这样问：

> 我刚学 C 语言，在运行这段代码时出现了下面的报错。请先告诉我报错是什么意思，再一步步带我检查，暂时不要直接给完整答案。

AI 的第一遍回答不一定有用。如果照着做以后出现了新的报错，就继续把新情况告诉它；如果没听懂，也可以直接说“讲得太难了，能不能用大一新生能听懂的方式再说一遍”。不用不好意思，它又不会嫌你烦。

### Agent 可以用在什么地方？

如果说普通 AI 像一个坐在旁边给你出主意的学长，那么 Agent 就像一个可以直接碰你电脑的学长——当然，它偶尔也会乱按。

Agent 可以直接打开你的项目文件，替你修改代码、运行程序，再根据新的报错继续调整。下面这些都是很实际的使用场景：

* **处理一个跑不起来的程序**：比如你的 C 程序编译失败，或者 Python 程序运行到一半报错，可以让 Agent 直接运行程序、读取完整报错、找到可能有问题的代码，修改后再运行一次；
* **直接修改代码**：比如给一个记账程序增加“删除记录”的功能，给网页加一个按钮，或者把程序输出的格式改得更好看。你只需要描述想要的效果，Agent 可以自己寻找需要修改的文件；
* **解决安装和环境问题**：遇到“缺少某个包”、版本不兼容或者项目不知道怎么启动时，可以让 Agent 阅读项目说明、安装需要的依赖并尝试运行；
* **看懂别人的代码**：从 GitHub 下载了一个项目却不知道从哪里看起时，可以让 Agent 先介绍每个文件夹的作用，再带你找到程序入口和最重要的几段代码；
* **处理重复工作**：例如给几十张图片重新命名、整理一批文件、统一修改文档格式，或者写一个小程序自动处理数据；
* **从头做一个小工具**：例如课程表网页、待办清单、简单计算器或批量整理文件的脚本。你负责说清楚想要什么，Agent 负责先做出一个能运行的版本，再根据你的意见继续修改。

如果一件事情需要“先看看文件，再改一点，运行一下，报错后接着改”，通常就比单纯聊天更适合交给 Agent。

### 怎样使用 Agent？

不同 Agent 的界面不太一样，但用起来大体都是这几步：

1. **把项目交给它**：打开需要处理的文件夹。重要文件最好先复制一份；如果已经会用 Git，就先建一个新分支；
2. **直接说想做什么**：比如“帮我看看这个项目为什么运行不了”“把这个按钮改成蓝色”；
3. **让它动手，但别完全不看**：它准备安装东西、删除文件或者访问其他内容时，先看一眼再点允许；
4. **改完自己试一下**：运行程序，看看功能是不是真的正常。如果不对，就把结果继续告诉它，让它接着改；
5. **确认以后再上传**：删除文件、提交作业、推送到 GitHub 或把内容发到网上之前，一定要自己确认。

例如，程序报错时可以直接这样说：

> 这是我的 C 语言作业，编译时出现了下面的报错。请先运行程序并找到原因，只修改和报错有关的代码；修改后再运行一次，然后用我能听懂的方式解释你改了什么。不要上传或提交任何内容。

第一次用时，先让 Agent 修改一段文档、解释一个文件或者修一个小报错即可。用过一两次之后，你自然就会知道该怎么和它交流了。

### 用的时候记住这几件事

AI 会犯错，而且经常错得很有自信，所以题目答案、重要资料和代码修改都要自己再看一遍。Agent 改过文件以后，也至少要亲自打开运行一下，别看到它说“已经完成”就直接交作业。

不要把账号密码、验证码、身份证照片、私人聊天记录和其他不想公开的内容发给 AI。也不要让 Agent 在你没看清的情况下删除文件、上传资料或者替你发送消息。

最后，课程允不允许使用 AI，要看老师的具体要求。允许使用也不代表可以直接把生成的内容当成自己的作业。至少要保证最后交上去的东西你自己看得懂，也能够向老师解释。
...
但出于各种原因，这里不做过多介绍。这点很重要，否则本文中的许多网站都无法进入，大部分比较前沿的大模型和Agent也需要科学上网才能使用。具体方法可以自行在网络上搜索，GitHub 上也有很多免费的资源<del>（不过不翻墙的话 GitHub 也可能上不去）</del>
...
* [**《提问的智慧》**](https://github.com/ryanhanwu/How-To-Ask-Questions-The-Smart-Way/blob/main/README-zh_CN.md)  
  正如我校沟通技巧课课件所言：<img src="https://cdn.tonycrane.cc/turing2022/images/v2-566c0540c037413964849ec7e13ce43f_1440w.jpeg" style="width:50%;"/>
...
<img src="https://cdn.tonycrane.cc/turing2022/images/XHPWlzDfEo4CAvT.png" style="width:40%;"/>
...
* 如果你很懒，不想写 LaTeX<del>（比如笔者）</del>可以利用 GitHub 上 <https://github.com/Keldos-Li/typora-latex-theme>提供的主题，将你的 typora 伪装成 LaTeX <img src="https://cdn.tonycrane.cc/turing2022/images/cover.png" style="width:100%;"/>
