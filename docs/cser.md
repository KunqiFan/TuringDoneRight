...
对于现在开始学习 CS 的同学，学会使用 AI 和 Agent 已经不是一项可有可无的“加分技能”，而是必须补上的基本能力。它们正在迅速进入搜索资料、阅读代码、调试程序、编写文档和完成项目的整个过程。就像我们需要学会使用搜索引擎、Git 和命令行一样，越早了解这些工具，就越早能够把时间用在真正需要思考的地方。

也请不要因为 AI 偶尔会胡说八道、有人拿它抄作业，或者觉得“自己写才算真正学会”，就对 AI 抱有芥蒂。真正需要避免的是盲目相信和过度依赖，尝试使用这些工具一定会使你的学习和工作效率大幅度提高。
...
不过，AI 说话流畅并不代表它真的知道答案。它有时会一本正经地胡说八道，编出不存在的函数、论文和网址。因此，AI 更适合帮你寻找思路、解释过程，而不是替你决定什么一定是对的。
...
比如做数学、物理题时，你可以把题目和自己的草稿拍下来，让 AI 读你的字迹，看看是从哪一步开始出错的。这里当然不是鼓励大家直接抄答案。比起问“这道题怎么做”，更推荐问“我做到这里为什么不对”“先给我一个提示”“这一步用了什么公式”。这样既能继续把题做下去，也不至于完全失去自己的思考过程。
...
不需要专门去背各种“高级提示词”，就把 AI 当成一个坐在旁边的学长，正常说话即可。不过，问题说得越清楚，它越容易帮上忙。

问问题时尽量做到下面三点：

1. **先说自己的情况**：比如“我是大一新生，刚学 C 语言，还没学过指针”；
2. **把信息发完整**：代码、题目、报错和自己的做法都尽量发出来，不要只说一句“怎么不行”；
3. **告诉它你想要什么**：是想直接解决问题，还是只要一个提示，或者希望它从最基础的地方开始解释。

例如，可以直接这样问：

> 我刚学 C 语言，在 Windows 上运行这段代码时出现了下面的报错。请先告诉我报错是什么意思，再一步步带我检查，暂时不要直接给完整答案。
...
Agent 可以直接打开你的项目文件，替你修改代码、运行程序，再根据新的报错继续调整。你可以让它：

* 看看一个刚下载下来的项目应该怎么运行；
* 帮你寻找代码为什么报错，并直接尝试修改；
* 给一个小功能补代码，或者给现有代码加上注释；
* 批量修改文件名、整理文档和处理重复工作；
* 按照你的描述做一个简单的网页或小工具。
...
> 请先看看这个项目，帮我解决运行时报错的问题。不要修改无关文件，也不要上传任何内容。改完以后请告诉我你改了哪里，并帮我运行一下。
